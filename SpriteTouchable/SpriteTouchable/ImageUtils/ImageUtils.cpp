...
bool ImageUtils::initQuotaFile(const int& numDayPasses){
    
    _quotaFilePath = CCFileUtils::sharedFileUtils()->getWritablePath()+"quota";
    // check if file exists first,
    if(CCFileUtils::sharedFileUtils()->isFileExist(_quotaFilePath.c_str())){
        // if file exists, leave it there
        return true;
    }
    else{
        // if file doesn't exist, initialize it with the numDayPasses
        // this might give the users a trick to increase quotas T_T
        
        FILE* fp = fopen(_quotaFilePath.c_str(), "w");
        if(fp){
            char buf[256];
            sprintf(buf, "%d\n%d\n", 10, 10);
            fputs(buf,fp);
...
bool ImageUtils::readQuotaFile(int& tipsQuota, int& solverQuota){
    _quotaFilePath = CCFileUtils::sharedFileUtils()->getWritablePath()+"quota";
    // check if file exists first,
    if(CCFileUtils::sharedFileUtils()->isFileExist(_quotaFilePath.c_str())){
        FILE* fp = fopen(_quotaFilePath.c_str(), "r");
        if(fp){
            char buf[256];
...
bool ImageUtils::writeQuotaFile(int& tipsQuota, int& solverQuota){
    _quotaFilePath = CCFileUtils::sharedFileUtils()->getWritablePath()+"quota";
    // check if file exists first,
    if(CCFileUtils::sharedFileUtils()->isFileExist(_quotaFilePath.c_str())){
        FILE* fp = fopen(_quotaFilePath.c_str(), "w");
